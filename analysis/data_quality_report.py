...
class DataQualityReport:
    ...
    def examine_structure(self):
        ...
        with open(self.output_path, "w") as f:
            ...
            f.write("\n📊 BASIC DATASET INFO\n")
            f.write("-" * 30 + "\n")
            f.write(f"Countries: {df.shape[0]} records\n")
            f.write(f"Indicators: {df.shape[1]} features\n")
            f.write(f"Data points: {df.shape[0] * df.shape[1]} total\n")

            # ! Health check
            missing_ptc = (df.isnull().sum().sum() / (df.shape[0] * df.shape[1])) * 100
            f.write(
